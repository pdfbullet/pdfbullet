...
const HeroBadge: React.FC<{ tool: Tool, animationClass: string, positionClasses: string }> = ({ tool, animationClass, positionClasses }) => (
    <div className={`absolute ${positionClasses} ${animationClass}`}>
        <Link 
            to={`/${tool.id}`} 
            title={`Open the ${tool.title} tool`}
...
const HomePage: React.FC = () => {
    ...
    const categories = useMemo(() => {
        const categoryOrder = ['organize', 'optimize', 'convert-to', 'convert-from', 'edit', 'security', 'business'];
        const categorySet = new Set(TOOLS.map(t => t.category).filter(Boolean) as string[]);
        const sortedCategories = categoryOrder.filter(cat => categorySet.has(cat));
    ...
    <div className="overflow-x-hidden">
        ...
        <section className="relative pt-16 pb-20 md:pt-20 md:pb-20 text-center bg-white dark:bg-black overflow-hidden">
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className="mt-8">
                    <Link to="/#all-tools" title="View All Tools" className="bg-brand-red hover:bg-brand-red-dark text-white font-bold py-3 px-6 sm:py-4 sm:px-8 rounded-lg text-base sm:text-lg transition-colors">
                        View All Tools
                    </Link>
                </div>
            </div>
        ...
        <section id="all-tools" className="bg-gray-50 dark:bg-black py-20">
            <div className="container mx-auto px-6">
                ...
                <div className="mb-16">
                    <h2 className="text-3xl font-bold text-gray-900 dark:text-gray-50 mb-6 scroll-mt-24">All Tools</h2>
                    <div className="flex flex-wrap justify-center gap-2 mb-8">
                        {categories.map(category => (
                            <button
                                key={category}
                                onClick={() => setActiveCategory(category)}
                                title={`Filter by ${category} tools`}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6">
                        {filteredTools.length > 0 ? filteredTools.map((tool) => (
                            ...
                            <p className="col-span-full text-center text-gray-500">No tools in this category.</p>
                        )}
                    </div>
                </div>
            </div>
        ...
        <section className="bg-white dark:bg-black py-20">
            <div className="container mx-auto px-6">
                ...
                <div className="mt-12 text-center">
                    <Link to="/how-to-use" title="Explore All Guides" className="inline-block bg-white dark:bg-black border-2 border-brand-red text-brand-red font-bold py-3 px-8 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                        Explore All Guides
                    </Link>
                </div>
            </div>
