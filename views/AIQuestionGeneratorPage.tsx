...
import { CopyIcon, BrainIcon, LeftArrowIcon } from '../components/icons.tsx';
...
const AIQuestionGeneratorPage: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const jsonResponse = await serverRes.json().catch(() => ({ error: 'Server error' }));

            if (!serverRes.ok) {
                throw new Error(jsonResponse.error || 'Failed to generate questions.');
            }
            ...
            if (errorMessage.toLowerCase().includes('api key') || errorMessage.toLowerCase().includes('configur')) {
                userFriendlyError = 'The AI service is currently unavailable. Please check your API key configuration on the server.';
    ...
    return (
        <div className="min-h-[calc(100vh-200px)] flex flex-col items-center py-12 px-4 sm:px-6 bg-gray-50 dark:bg-black relative">
            ...
            <div className="text-center mb-10">
                <h1 className={`text-4xl font-extrabold ${tool.textColor}`}>{t(tool.title)}</h1>
                <p className="mt-2 text-lg text-gray-600 dark:text-gray-300 max-w-2xl">{t(tool.description)}</p>
            </div>
