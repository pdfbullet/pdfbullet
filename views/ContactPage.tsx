...
const ContactPage: React.FC = () => {
  ...
  const dynamic = content?.contactPage || {};

  useEffect(() => {
    document.title = dynamic.metaTitle || "Contact Us | PDFBullet Support";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
        metaDesc.setAttribute("content", dynamic.metaDescription || "Get in touch with the PDFBullet team. Contact us for support, suggestions, or any questions you have about our free online PDF tools.");
    }
  }, [dynamic]);
      ...
      <div className="px-6">
        <div className="max-w-4xl mx-auto text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-800 dark:text-gray-100">
            {dynamic.heroHeadline || "Get In Touch"}
          </h1>
          <p className="mt-4 text-lg text-gray-600 dark:text-gray-300">
            {dynamic.heroSubheadline || "Our dedicated support team is here to assist you with any inquiries, feedback, or technical issues. We strive to provide timely and helpful responses to ensure you have the best experience with our tools."}
          </p>
        </div>
        ...
        <div className="max-w-5xl mx-auto mb-8 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white dark:bg-black border border-gray-200 dark:border-gray-800 p-6 rounded-lg shadow-md flex items-center gap-4 border-glow-hover">
                 ...
                 <div>
                    <h3 className="font-semibold text-gray-800 dark:text-gray-100">
                        {dynamic.emailTitle || "Email Us"}
                    </h3>
                    <a href={`mailto:${dynamic.emailValue || 'support@pdfbullet.com'}`} className="text-brand-red hover:underline">
                        {dynamic.emailValue || "support@pdfbullet.com"}
                    </a>
                 </div>
            </div>
            <a href="https://wa.me/message/JYA22CVSYSZ4N1" target="_blank" rel="noopener noreferrer" className="bg-white dark:bg-black border border-gray-200 dark:border-gray-800 p-6 rounded-lg shadow-md flex items-center gap-4 border-glow-hover hover:bg-gray-50 dark:hover:bg-gray-800">
                 ...
                 <div>
                    <h3 className="font-semibold text-gray-800 dark:text-gray-100">
                        {dynamic.whatsappTitle || "WhatsApp"}
                    </h3>
                    <p className="text-gray-500 dark:text-gray-400">
                        {dynamic.whatsappValue || "Chat with us"}
                    </p>
                 </div>
            </a>
            <div className="bg-white dark:bg-black border border-gray-200 dark:border-gray-800 p-6 rounded-lg shadow-md flex items-center gap-4 border-glow-hover">
                 ...
                 <div>
                    <h3 className="font-semibold text-gray-800 dark:text-gray-100">
                        {dynamic.addressTitle || "Office Address"}
                    </h3>
                    <p className="text-gray-500 dark:text-gray-400">
                        {dynamic.addressValue || "Kathmandu, Nepal"}
                    </p>
                 </div>
            </div>
        </div>
          ...
          {submitted ? (
            ...
            <form action="https://formspree.io/f/mwpqenrr" method="POST" onSubmit={handleSubmit}>
               <h3 className="text-xl font-bold text-center text-gray-800 dark:text-gray-100 mb-6">
                    {dynamic.formTitle || "Or send us a message directly"}
               </h3>
              ...
              <div className="mt-8 text-center">
                <button type="submit" disabled={submitting} className="bg-brand-red hover:bg-brand-red-dark text-white font-bold py-3 px-10 rounded-md transition-colors text-lg disabled:bg-red-300 dark:disabled:bg-red-800">
                  {submitting ? 'Sending...' : (dynamic.formSubmitText || 'Send Message')}
                </button>
              </div>
            </form>
          )}
