...
const Header: React.FC<HeaderProps> = ({ onOpenProfileImageModal, onOpenSearchModal, onOpenChangePasswordModal, onOpenQrCodeModal, isPwa, unreadCount, justReceivedNotification, onNotificationAnimationEnd }) => {
  ...
  // FIX: Replace deprecated 'isPremium' with 'isToolsPremium' for trial check.
  const onTrial = user && !user.isToolsPremium && user.trialEnds && user.trialEnds > Date.now();
  ...
  const desktopNavLinks = [
    { to: '/flipbooks', labelKey: 'flipbooks' },
    { to: '/pdf-reader', labelKey: 'pdf reader' },
    { to: '/compress-pdf', labelKey: 'compress pdf' },
  ];
  ...
  const desktopGridMenuData = useMemo(() => ({
    products: [
      { title: 'Pdf Bullet Sign', description: 'e-Signing made simple', to: '/workflows', icon: HeartbeatIcon, iconColor: 'bg-blue-600' },
      { title: 'Pdf Bullet API', description: 'Document automation for developers', to: '/developer', icon: HeartbeatIcon, iconColor: 'bg-teal-500' },
    ],
    solutions: [
    ...
    applications: [
      ...
      { 
        title: 'Mobile App', 
        description: 'Download the from PlayStore', 
  ...
  const imageToolIds = new Set(['remove-background']);
      ...
      <div className="px-4 sm:px-6 py-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            {user ? (
               <>
                <p className="hidden lg:block text-sm font-semibold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-pink-500 dark:from-purple-400 dark:to-pink-400 whitespace-nowrap">
                    ...
                    <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                      <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{t('header.signed_in_as')}</p>
                       <div className="flex items-center justify-between mt-1">
                          ...
                          {/* FIX: Replace deprecated 'isPremium' with a check for 'isToolsPremium' or 'isFlipbookPremium'. */}
                ...
                <div className="absolute top-full right-0 mt-2 z-20">
                  <div className="w-[calc(100vw-2rem)] max-w-sm md:max-w-2xl lg:max-w-[52rem] bg-white dark:bg-black border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl overflow-hidden">
                    {gridMenuView === 'main' && (
                     ...
                     <div className="bg-gray-50 dark:bg-gray-900/50 p-3 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
            ...
            <nav className="space-y-4">
                <div>
                  <button onClick={() => toggleAccordion('tools')} className="w-full flex justify-between items-center font-bold text-lg text-gray-800 dark:text-gray-100">
                   ...
                   <div className={`overflow-hidden transition-all duration-300 ${openAccordion === 'tools' ? 'max-h-[1000px] mt-2' : 'max-h-0'}`}>
                        {mobileMenuStructure.map((category: MobileMenuCategory) => (
                          <div key={category.title}>
                             <h4 className="font-semibold text-gray-500 mt-3 mb-1">{category.title}</h4>
                              <div className="space-y-1">
                                {category.tools.map((tool: Tool) => (
                                    <Link key={tool.id} to={`/${tool.id}`} onClick={closeAllMenus} title={t(tool.title)} className="flex items-center gap-3 p-2 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800">
                                        <tool.Icon className={`h-5 w-5 flex-shrink-0 ${tool.textColor}`} />
                                        <span className="font-semibold text-sm">{t(tool.title)}</span>
                                    </Link>
                                ))}
                ...
                <div><Link to="/developer" onClick={closeAllMenus} className="font-bold text-lg text-gray-800 dark:text-gray-100">Developer</Link></div>
                 <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                    <div className="w-full flex justify-between items-center font-bold text-lg text-gray-800 dark:text-gray-100">
                        <span>App Mode</span>
                        <ToggleSwitch checked={isAppMode} onChange={handleViewModeToggle} />
                    </div>
                </div>
