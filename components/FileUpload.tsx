...
import { UploadCloudIcon, DropboxIcon, PlusIcon, CloseIcon, DesktopIcon } from './icons.tsx';
...
const FileUpload: React.FC<FileUploadProps> = ({ tool, files, setFiles, accept, children }) => {
  // Component removed gapi and pickerApiLoaded states and effect.

  const onDrop = useCallback((acceptedFiles: File[], fileRejections?: any[]) => {
    const rejections = fileRejections || [];
    console.log('FileUpload onDrop called. Accepted:', acceptedFiles.map(f => f.name), 'Rejected:', rejections.map(r => `${r.file.name}: ${r.errors.map((e: any) => e.message).join(', ')}`));
  ...
  // Removed Google Drive picker methods.
