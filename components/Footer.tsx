...
const Footer: React.FC<FooterProps> = ({ onOpenCalendarModal }) => {
  ...
  const { t } = useI18n();
  const { promptInstall, isPwa } = usePWAInstall();
  ...
  const handleDownloadLogo = () => {
    ...
    link.href = '/logo.svg';
    link.download = 'pdf-bullet-logo.svg';
  ...
  const socialAndAppLinks = (
      <>
        ...
        {!isPwa && (
          <div className="mt-6">
              <p className="text-gray-400 text-sm font-semibold mb-3">Get the app</p>
              <div className="flex items-center gap-4">
                <a href="https://play.google.com/store/apps/details?id=com.pdfbullet.app" target="_blank" rel="noopener noreferrer" className="inline-block transition-transform hover:scale-105" aria-label="Get it on Google Play">
                    <img src="https://ik.imagekit.io/fonepay/playstore.png?updatedAt=1760235759138" alt="Get it on Google Play" className="h-12 w-auto" />
                </a>
                <button onClick={promptInstall} className="inline-block transition-transform hover:scale-105" aria-label="Download on the App Store">
                    <img src="https://ik.imagekit.io/fonepay/appstore.png?updatedAt=1760235759216" alt="Download on the App Store" className="h-12 w-auto" />
                </button>
              </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-8">
          ...
          <div><h3 className="font-bold text-lg mb-4">{t('footer.company')}</h3>
            <ul className="space-y-2 text-gray-400 text-sm">
                ...
                <li><Link to="/submit-ticket" title={t('footer.company_links.report_problem')} className="hover:text-white text-left">{t('footer.company_links.report_problem')}</Link></li>
            </ul>
          </div>
