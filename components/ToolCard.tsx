...
interface ToolCardProps {
  ...
  disabled?: boolean;
  filesToPass?: File[]; // Prop passed from PwaToolsPage, keep for type safety
}

const ToolCard: React.FC<ToolCardProps> = ({ tool, isFavorite, onToggleFavorite, disabled }) => {
  ...
  const linkClasses = `
    ...
    rounded-xl shadow-md h-36 group
    ${disabled 
  ...
  return (
    <Link 
      ...
      title={disabled ? 'This tool is not compatible with the selected file type' : `Open the ${t(title)} tool`}
