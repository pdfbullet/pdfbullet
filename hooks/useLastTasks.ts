...
export const useLastTasks = () => {
    ...
    const addTask = useCallback(async (task: Omit<LastTask, 'id' | 'timestamp' | 'dueDate'>) => {
        const now = Date.now();
    ...
    const updateTaskDueDate = useCallback(async (id: number, dueDate: string | null) => {
        try {
            const db = await getDb();
            const tx = db.transaction(TASK_STORE_NAME, 'readwrite');
            const store = tx.objectStore(TASK_STORE_NAME);
            const task = await new Promise<LastTask | undefined>((resolve, reject) => {
            ...
            if (task) {
                const updatedTask: LastTask = { ...task };
                if (dueDate) {
            ...
            console.error("Failed to update task due date in IndexedDB", error);
        }
    }, [loadTasks]);

    return { tasks, loading, addTask, deleteTask, updateTaskDueDate };
};